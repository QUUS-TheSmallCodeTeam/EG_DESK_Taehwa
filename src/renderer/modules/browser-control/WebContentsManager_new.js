...
class WebContentsManager {
  constructor() {
    ...
    this.webContentsViews = new Map(); // Using WebContentsView instead of BrowserView
    this.currentTabId = null;
    this.mainWindow = null;
    this.baseWindow = null; // WebContentsView uses BaseWindow
  ...
  /**
   * Create a new browser tab/view using WebContentsView
  ...
  async switchTab(tabId) {
    ...
    // Hide current tab if any (using the new API)
    if (this.currentTabId && this.currentTabId !== tabId) {
      // For WebContentsView, we need to remove from the window's contentView
      if (this.mainWindow.contentView) {
        this.mainWindow.contentView.removeChildView(this.webContentsViews.get(this.currentTabId));
      }
    }

    // Show the requested tab (using the new WebContentsView API)
    if (!this.mainWindow.contentView.children.includes(webContentsView)) {
  ...
  async goBack(tabId = null) {
    ...
    if (webContentsView && webContentsView.webContents.canGoBack()) {
  ...
  async goForward(tabId = null) {
    ...
    if (webContentsView && webContentsView.webContents.canGoForward()) {
  ...
  getNavigationState(tabId = null) {
    ...
    return {
      canGoBack: webContentsView.webContents.canGoBack(),
      canGoForward: webContentsView.webContents.canGoForward(),
  ...
  /**
   * Update WebContentsView bounds (different from BrowserView)
   */
  updateWebContentsViewBounds(preciseBounds = null) {
    ...
    if (preciseBounds) {
      ...
      // WebContentsView should fill the .browser-viewport area inside BrowserTabComponent
      newBounds = {
        x: containerStartX,
    ...
    // WebContentsView uses setBounds method like BrowserView
    webContentsView.setBounds(newBounds);
    console.log(`[WebContentsManager] Updated WebContentsView bounds: ${JSON.stringify(newBounds)}`);
  }

  /**
   * Set up event handlers for a WebContentsView
   */
  setupWebContentsViewEvents(tabId, webContentsView) {
